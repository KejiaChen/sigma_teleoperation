...
class EEVelocityCmdLogger(Node):
    def __init__(self):
        super().__init__('ee_velocity_cmd_logger')

        self.declare_parameter("save_interval", 0.05)
        self.declare_parameter("topic_name", "/frankaRight/ee_velocity_cmd")
        ...
        self.save_dir = "/docker_volume/ros2_ws/src/joint_state_collection/ee_velocity_cmd_data"
        ...
        self.get_logger().info("[EE Velocity Cmd Logger] Ready. Press '1' to start, '2' to stop logging.")
    ...
    def start_logging(self):
        if not self.recording:
            ...
            prefix = f"{date_str}_"
            suffix_full = "ee_velocity_cmd_right_full.txt"
            suffix_synced = "ee_velocity_cmd_right_synced.txt"
            ...
            base_filename = f"{date_str}_{next_index}_ee_velocity_cmd_right"
            ...
            self.recording = True
            self.get_logger().info(f"Started EE velocity Cmd logging: {base_filename}")
    ...
    def stop_logging(self):
        if self.recording:
            ...
            self.recording = False
            self.get_logger().info("Stopped EE velocity Cmd logging.")
