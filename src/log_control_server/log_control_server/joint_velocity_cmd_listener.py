...
class JointVelocityCmdLogger(Node):
    def __init__(self):
        super().__init__('joint_velocity_cmd_logger')

        self.declare_parameter("save_interval", 0.05)
        self.declare_parameter("topic_name", "/frankaRight/joint_velocity_cmd")
        ...
        self.save_dir = "/docker_volume/ros2_ws/src/joint_state_collection/joint_velocity_cmd_data"
        ...
        self.get_logger().info("[Velocity Cmd Logger] Ready. Press '1' to start, '2' to stop recording.")
    ...
    def write_to_file(self, file_handle, msg):
        ...
        line = f"{timestamp},{name_str},{vel_str}\n"
    ...
    def start_logging(self):
        if not self.recording:
            ...
            prefix = f"{date_str}_"
            suffix_full = "joint_velocity_cmd_right_full.txt"
            suffix_synced = "joint_velocity_cmd_right_synced.txt"
            ...
            base_filename = f"{date_str}_{next_index}_joint_velocity_cmd_right"
            ...
            header = "timestamp,name,velocity\n"
            self.synced_file.write(header)
            self.realtime_file.write(header)

            self.recording = True
            self.get_logger().info(f"Started Velocity cmd logging: {base_filename}")
    ...
    def stop_logging(self):
        if self.recording:
            ...
            self.recording = False
            self.get_logger().info("Stopped Velocity cmd logging.")
