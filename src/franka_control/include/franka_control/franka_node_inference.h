...
class FrankaNodeInference : public rclcpp::Node {
   public:
    FrankaNodeInference(std::string node_name, std::string robot_IP);
    ~FrankaNodeInference();
   ...
   private:
    ...
    Eigen::Vector3d pending_absolute_position_ = Eigen::Vector3d::Zero();
    Eigen::Quaterniond pending_absolute_orientation_ = Eigen::Quaterniond::Identity();
